...
import xy177.inspectanimationsantique.config.InspectConfig;
import xy177.inspectanimationsantique.network.AnimationRequestMessage;
...
public final class ClientAnimationController {
    ...
    public static void handleKeys() {
        ...
        while (ClientKeyBindings.CONFIG.isPressed()) {
            minecraft.displayGuiScreen(new GuiConfig(minecraft.currentScreen,
                    InspectAnimationsAntique.MOD_ID, InspectAnimationsAntique.NAME));
        }
        if (player == null) {
            return;
        }
        ...
        while (ClientKeyBindings.INSPECT.isPressed()) {
            if (ClientKeyBindings.MODIFIER.isKeyDown()) {
                ...
                player.sendStatusMessage(new TextComponentTranslation(
                        "text." + InspectAnimationsAntique.MOD_ID + ".switch_animation")
                        .appendText(" ")
    ...
    public static AnimationState state(AbstractClientPlayer player) {
        return ((IAnimationState) player).inspectAnimationsAntique$getAnimationState();
    }
